...
func (h *Handler) HandlePostActivate(c *echo.Context) error {
	...
	if err := h.CheckSessionUsername(c); err != nil {
		...
		return c.JSON(err.HttpStatusCode, map[string]interface{}{"status": "failed", "message": err.Details})
	}
	...
	if err := h.AddToDB(anchor); err != nil {
		...
		return c.JSON(http.StatusInternalServerError, map[string]interface{}{"status": "failed", "message": msg})
	}
	...
	return c.JSON(http.StatusCreated, map[string]interface{}{"status": "activated", "user": anchor.Username, "message": "authpf anchor is being loaded"})
}
...
func (h *Handler) HandleGetActivate(c *echo.Context) error {
	...
	if err := h.CheckSessionUserPermission(c, config.SESSION_VIEW); err != nil {
		c.Set("authpf", err.Message)
		return c.JSON(err.HttpStatusCode, map[string]interface{}{"status": "rejected", "message": err.Details})
	}
...
func (h *Handler) HandleDeleteDeactivate(c *echo.Context) error {
	...
	if !check {
		...
		return c.JSON(http.StatusForbidden, map[string]interface{}{"status": "rejected", "user": anchor.Username, "message": msg})
	}

	// Exec
	if err := h.CallExecDeactivateAnchor(anchor); err != nil {
		c.Set("authpf", err.Details)
		return c.JSON(err.HttpStatusCode, map[string]interface{}{"status": "failed", "message": err.Message})
	}
	...
	return c.JSON(http.StatusAccepted, map[string]interface{}{"status": "queued", "user": anchor.Username, "message": msg})
}
...
func (h *Handler) HandleDeleteAllDeactivate(c *echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]interface{}{"status": "cleared"})
}
